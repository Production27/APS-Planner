...
// counters in users.js: infrequent enough that a new binding isn't worth
...
// as sanitizeJobCommentAuthors() in room-state.js: prefer the server's
...
// username field.
export async function handleReportError(request, env, corsHeaders) {
  let body;
...
export async function handleErrorsList(request, env, corsHeaders) {
  let body;
  try { body = await request.json(); } catch (e) { return jsonResponse({ error: "Invalid JSON body" }, 400, corsHeaders); }
